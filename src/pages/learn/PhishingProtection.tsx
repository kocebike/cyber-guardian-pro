...
import { Eye, Mail, AlertTriangle, Shield } from 'lucide-react';
...
const PhishingProtection = () => {
  ...
  const examples = {
    bg: [
      { type: 'phishing', from: 'security@amaz0n-account.com', subject: 'СПЕШНО: Акаунтът ви е компрометиран!', body: 'Кликнете тук незабавно за да защитите акаунта си...' },
      { type: 'legitimate', from: 'no-reply@amazon.com', subject: 'Потвърждение на поръчка #123-456', body: 'Благодарим ви за поръчката. Можете да проверите статуса в акаунта си.' },
    ],
    en: [
      { type: 'phishing', from: 'security@amaz0n-account.com', subject: 'URGENT: Your account has been compromised!', body: 'Click here immediately to protect your account...' },
      { type: 'legitimate', from: 'no-reply@amazon.com', subject: 'Order confirmation #123-456', body: 'Thank you for your order. You can check the status in your account.' },
    ],
  };
  ...
  const content = {
    bg: {
      ...
      sections: [
        { title: 'Какво е фишинг?', content: `Фишингът е вид кибератака, при която престъпниците се представят за легитимни организации, за да откраднат вашите данни.
...
📞 Vishing - телефонни обаждания от "техническа поддръжка"` },
        { title: 'Как да разпознаете фишинг имейл?', highlight: 'warning' as const, content: `⚠️ Подозрителен имейл адрес
...
⚠️ Линкове към фалшиви сайтове
   Задръжте курсора върху линка преди да кликнете` },
        { title: 'Как да проверите линк?', highlight: 'tip' as const, content: `1. Задръжте курсора върху линка (без да кликвате)
   Проверете URL адреса в долния ляв ъгъл

2. Търсете HTTPS и катинарче
   HTTPS не означава автоматично безопасен сайт!
...
4. Използвайте VirusTotal.com за проверка на линкове` },
        { title: 'Какво да правите при фишинг?', highlight: 'success' as const, content: `1. НЕ КЛИКВАЙТЕ върху линкове
...
• Свържете се с банката си` },
    ...
    en: {
      ...
      sections: [
        { title: 'What is phishing?', content: `Phishing is a type of cyber attack where criminals pretend to be legitimate organizations to steal your data.
...
📞 Vishing - phone calls from "technical support"` },
        { title: 'How to recognize a phishing email?', highlight: 'warning' as const, content: `⚠️ Suspicious email address
...
⚠️ Links to fake sites
   Hover over the link before clicking` },
        { title: 'How to verify a link?', highlight: 'tip' as const, content: `1. Hover over the link (without clicking)
...
4. Use VirusTotal.com to check links` },
        { title: 'What to do about phishing?', highlight: 'success' as const, content: `1. DON'T click on links
...
• Contact your bank` },
  ...
  const currentExamples = examples[language as keyof typeof examples] || examples.bg;
      ...
      <CardContent className="p-6 md:p-8">
        ...
        <div className="space-y-4">
          {currentExamples.map((example, index) => (
            <div key={index} className={`p-4 rounded-lg border ${example.type === 'phishing' ? 'bg-destructive/10 border-destructive/30' : 'bg-primary/10 border-primary/30'}`}>
              <div className="flex items-center gap-2 mb-2">
                {example.type === 'phishing' ? <AlertTriangle className="h-5 w-5 text-destructive" /> : <Shield className="h-5 w-5 text-primary" />}
                <span className={`text-sm font-bold ${example.type === 'phishing' ? 'text-destructive' : 'text-primary'}`}>
                  {example.type === 'phishing' ? (language === 'bg' ? 'ФИШИНГ' : 'PHISHING') : (language === 'bg' ? 'ЛЕГИТИМЕН' : 'LEGITIMATE')}
                </span>
              </div>
  ...
  return (
    <LearnModulePage
      ...
      isPremium={false}
      prevLink="/learn/password-security"
      nextLink="/learn/2fa-setup"
